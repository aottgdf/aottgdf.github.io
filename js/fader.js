    ...
    */
    me.fade = function(config) {
        ...
        if (config.returnType === 'hex') {
            ...
            for (ii = 0; ii < len; ii++) {
                color = colors[ii];
                colors[ii] = numSign + decToHex( color.r ) + decToHex( color.g ) + decToHex( color.b );
            }
        }
...
var TextFader = TextFader || (function(fader) {
    var me = {};
    ...
    /*
        ...
        * output - 'html-span', 'html-font', 'ezboard', 'vbulletin', 'forum'
    */
    me.fade = function(config) {
        var colors = config.colors,
            ...
            txtLines = txt.split('\n'),
            openTagStart, openTagEnd, closeTag, lineStart, lineEnd,numSign = true,
            ii, jj, cIndex, elm,
            len,
            len2,
            fadeLen = 0,
            ...
            ret = '',
            spinForward,
            spinSpeed = (typeof config.spinSpeed !== 'undefined') ? parseInt(config.spinSpeed, 10) : 1;
console.log(output)
        switch(output) {
            case 'tag':
                openTagStart = '[';
        ...
        switch (type) {
            ...
            case 'horizontal':
                ...
                for (ii = 0; ii < len; ii++) {
                    ...
                    ret += lineStart;
                    for (jj = 0; jj < len2; jj++) {
                        if ( txtLines[ii].substr(jj,1) === ' ' && (output === 'vbulletin' || output === 'forum')) {
